...
public class MultiPolynomial {
    ...
    каждому из которых соответствует своя переменная. Имя переменной определяется её позицией в списке.
    Таким образом, если мы хотим,
    чтобы в полиноме были только слагаемые с переменной v2, мы должны передать 2 пустых массива, а потом тот,
    ...
    - Выбираем, какой из мультиполиномов длинее по количеству простых полиномов
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof MultiPolynomial) {
            ...
            // их также необходимо просуммировать, при этом в этих полиномах может быть только коэффициент
